...
def create_role(name, is_system=0):
    return Role.objects.create(
        name=name, is_system=is_system, is_blocked=False, audit_user_id=-1
    )
...
class RightsCacheInvalidationTest(TestCase):
    ...
    def test_uba_right_stays_out_of_the_global_bag_after_invalidation(self):
        # the global bag depends on RoleRight.uba, it has to be recomputed accordingly
        RoleRight.objects.create(
            role=self.role, right_id=ADDED_RIGHT_ID, uba=True, audit_user_id=-1
        )

        self.assertNotIn(ADDED_RIGHT_ID, self.i_user.rights)
        self.assertIn(ADDED_RIGHT_ID, self.i_user.rights_for_uba)
    ...
    def test_assigning_admin_role_invalidates_is_admin_cache(self):
        admin_role = create_role("TestRightsCacheAdminRole", is_system=64)
...
class PatternCacheStub:
    """Cache double exposing `delete_pattern` only when the backend supports it."""
...
class RoleChangeCacheInvalidationTest(TestCase):
    ...
    def test_role_change_deletes_rights_keys_by_pattern_when_supported(self):
        ...
        self.assertFalse(stub.deleted_keys)

    def test_role_change_deletes_the_holders_keys_when_patterns_unsupported(self):
        ...
        # no wildcard, and no cache.clear() that would also flush the object cache
        self.assertFalse(stub.deleted_patterns)
        self.assertFalse(stub.cleared)
        self.assertIn(f"rights_{user.i_user.id}", stub.deleted_keys)
        self.assertIn(f"is_admin_{user.i_user.id}", stub.deleted_keys)

    def test_user_role_change_never_uses_a_pattern(self):
        user = create_test_interactive_user(username="TestUserRoleCacheUser")
        stub = PatternCacheStub(supports_patterns=True)

        with patch("core.receivers.cache", stub):
            UserRole.objects.create(
                user=user.i_user, role=self.role, audit_user_id=-1
            )

        self.assertFalse(stub.deleted_patterns)
        self.assertEqual(
            [f"rights_{user.i_user.id}", f"is_admin_{user.i_user.id}"],
            stub.deleted_keys,
        )
