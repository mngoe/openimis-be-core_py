    ...
    def inner_function(async_mutate):
        ...
        def wrapper(cls, user, **data):
            if not data.get("uuids", None):
                ...
                if user is None:
                    raise Exception(
                        "Veuillez s'il vous plaît rafraîchir votre page avant d'effectuer cette action.")
    ...
    def inner_function(async_mutate):
        ...
        def wrapper(cls, user, **data):
            if not data.get("uuids", None):
                ...
                logger.warning("q_filter %s ", q_filter)
                logger.warning("User: %s ", user)

                incoming_qs = data.get(queryset_key)
                logger.warning("incoming_qs %s ", incoming_qs)
                ...
                logger.warning("base_query %s ", base_query)
